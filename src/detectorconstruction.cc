...
//                                 "det_phy", worldPhysical->GetLogicalVolume(), false, 0, false);
    G4double fraction = 1./(81+35.3);
    det_phys = new G4PVPlacement(0, G4ThreeVector(0,80,60)*cm,
                      new G4LogicalVolume(new G4Box("dap",30.61*cm*fraction*0.5,0.05*cm,39.54*cm*fraction*0.5),
                      G4NistManager::Instance()->FindOrBuildMaterial("G4_AIR"),"dap"),
...
void DetectorConstruction::SetupWorldGeometry()
{
    ...
    container_logic = new G4LogicalVolume(container_sol, vacuum, "phantomLogical");
...
{
    auto sd = new MeshSD("phantomSD","pshit");
    G4SDManager::GetSDMpointer()->AddNewDetector(sd);

    SetSensitiveDetector("phantomLogical",sd);

    auto mfd = new G4MultiFunctionalDetector("dap");
    mfd ->RegisterPrimitive(new G4PSDoseDeposit("dose"));
    G4SDManager::GetSDMpointer()->AddNewDetector(mfd);
    SetSensitiveDetector(det_phys->GetLogicalVolume(),mfd);
}
